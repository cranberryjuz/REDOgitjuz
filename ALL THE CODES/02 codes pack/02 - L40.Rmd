...
```
#Data Frame
Get data for each column, as vector
...
Class_vec <- data.frame(student, marks, Rank, Passed)
str(Class_DF)
```


Putting the data tgt to create 
```{r}
class_df <- data.frame (studetn_vec,marks_vec,passed_vec,rank_vec)
class_df
```
